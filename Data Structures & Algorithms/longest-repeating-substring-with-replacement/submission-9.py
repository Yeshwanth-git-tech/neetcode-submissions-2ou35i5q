class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        #     #maxfreq = max(count[s[r]])
        #     #here we dont need to check the maxf and update it , as the maxf if it become less is fine and it increases the conditon below will take care of it - O(n)
        #     # while (r-l+1) - maxf > k:
...
#      window = 6-3+1 = 4, maxl stays 5
        ...
        for r in range(len(s)):
            ...
            # I don't decrease maxf when shrinking. That's deliberate, and it's safe because a stale maxf only makes the shrink condition stricter, so it can never produce a longer answer than the real one.
